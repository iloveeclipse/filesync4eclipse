...
public class RateIt {
    ...
    static boolean isProxyEnabled() {
        ServiceTracker proxyTracker = new ServiceTracker(FileSyncPlugin.getDefault()
    ...
    static boolean rate(int value, String comment) {
        ...
        try {
            ...
            String content = "rating=" + value + "&ratinglid=558&ratinguser=outside"
                    + "&req=addrating";
            ...
            FileSyncPlugin.error("Failed to rate on EPIC...", e);
            return false;
        } finally {
            if (out != null) {
                try {
                    ...
                    FileSyncPlugin.error("Failed to close stream...", e);
                }
            }
            if (input != null) {
                try {
        ...
        if (ratingDone) {
            ...
            textArea
                    .setText("Unfortunately,\nhttp proxy "
                            + "is enabled and therefore you can't rate FileSync plugin now...\n"
                            + "Please try to rate for FileSync plugin\nfrom another workstation,\nuse this url:\n"
                            + "www.eclipseplugincentral.com/Web_Links-index-req-ratelink-lid-558-ttitle-FileSync.html");
            combo.setEnabled(false);
            textArea.setEditable(false);
            okButton.setEnabled(false);
        }
