...
public final class FS {
    ...
    public static final boolean CLOSE_WHEN_DONE = true;
    ...
    public static boolean create(File destination, boolean isFile) {
        ...
        if (isFile) {
            ...
            if (!dir.exists()) {
                ...
                if (!result) {
                    if (enableLogging) {
                        FileSyncPlugin.log("Could not create directory '" + dir + "'", null,
                                IStatus.WARNING);
                    }
                    return false;
                }
            ...
            if (!result && enableLogging) {
                FileSyncPlugin.log("Could not create directory '" + destination + "'", null,
                        IStatus.WARNING);
            }
        }
        return result;
    ...
    public static boolean delete(File source, boolean recursive) {
        ...
        if (recursive) {
             ...
             * comented out because this kind of decisions is for SyncWizard only else { File dir
             * = source.getParentFile(); boolean ok = delete(source, false); if(ok &&
             * dir.list().length == 0){ // delete parant directory, because it is now empty // and
             * we have a clean build ok = delete(dir, false); } return ok; }
             */
        }
        boolean result = source.delete();
        if (!result && !source.isDirectory() && enableLogging) {
            FileSyncPlugin.log("Could not delete file '" + source + "'", null, IStatus.WARNING);
        }
        return result;
    }
    ...
    public static boolean copy(File source, File destination,
        ...
        if (source == null || destination == null || !source.exists() || !destination.exists()
                || source.isDirectory() || destination.isDirectory()) {
            if (enableLogging) {
                FileSyncPlugin.log("Could not copy file '" + source + "' to '" + destination + "'",
        ...
        /*
         * prevent from overhead on identical files - this works fine <b>only</b> if source and
         * destination are on the same partition (=> the same filesystem). If both files are on
         * different partitions, then 1) the file size could differ because of different chunk size
         * 2) the file time could differ because of different timestamp formats on different file
         * systems (e.g. NTFS and FAT)
         */
        ...
        try {
            ...
            if (fin != null) {
                try {
                    ...
                    if (enableLogging) {
                        FileSyncPlugin.log("Could not close file stream for file '" + source + "'",
                                e, IStatus.WARNING);
                    }
                    success = false;
                }
            ...
            if (fout != null) {
                try {
                    ...
                    if (enableLogging) {
                        FileSyncPlugin.log("Could not close file stream for file '" + destination
                                + "'", e, IStatus.WARNING);
                    }
                    success = false;
                }
